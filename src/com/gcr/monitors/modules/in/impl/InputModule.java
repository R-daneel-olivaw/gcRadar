...
public abstract class InputModule implements InputModuleInterface {
	...
	public <I> boolean addObject(I object, String identifier,
		...
		synchronized (watchList) {
			...
			if (!historyMap.containsKey(identifier)) {
				AbstractObjectRefrenceKey<Object> refrenceKey = new SequentialObjectRefrenceKey<Object>(
	...
	public <I> boolean addObject(I object, GcRadarCallback callback) {
		...
		String autoGenKey = object.getClass().getName() + " - "
				+ incrementEntryCounter();
	 ...
	 * @since 0.2
	 */
	public boolean addAll(Collection<?> collection, String identifier,
			GcRadarCallback callback) {
		if (collection == null) {
	...
	public boolean removeAll(
			Collection<? extends AbstractObjectRefrenceKey<?>> keyCollection) {
		if (keyCollection == null) {
			throw new IllegalArgumentException("Key collection can not be null");
		}

		Iterator<? extends AbstractObjectRefrenceKey<?>> iterator = keyCollection
				.iterator();
	 ...
	 * 
	 */
	private class SequentialObjectRefrenceKey<T> extends
			AbstractObjectRefrenceKey<T> {
		 ...
		 *            events
		 */
		public SequentialObjectRefrenceKey(T object, String identifier,
