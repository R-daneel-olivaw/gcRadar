...
public abstract class TreeInputModule implements InputModuleInterface
{
    ...
    private GcRadarAnnotationScanner annotationSacnner;
	...
	if (!historyMap.containsKey(identifier))
	{
	    ...
	    List<? extends FieldNameValuePair<? extends Object>> allFields = annotationSacnner.getAllFields(object, identifier,
		    isOptimistic);

	    synchronized (watchList)
	    {
		// Add the object itself
		AbstractObjectRefrenceKey<Object> refrenceKey = new TreeObjectRefrenceKey<Object>(object, identifier,
		...
		{
		    refrenceKey = new TreeObjectRefrenceKey<Object>(fnPair.getRefrenceValue(), fnPair.getFieldName(),
			    weakReferenceQueue, phantomReferenceQueue, callback);

		    watchList.add(refrenceKey);

		    historyMap.put(identifier + "[" + counter + "]", refrenceKey);
	 ...
	 *            events
	 */
	public TreeObjectRefrenceKey(T object, String identifier, ReferenceQueue<Object> weakReferenceQueue,
